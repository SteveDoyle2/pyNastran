...
\section{Getting Started: BDF Reading}
 ...
 \subsection{{\tt writeMesh.py}: Writing the BDF}
     ...
     Many cards also have a `write_bdf(...)` and `write_bdf2(...)` method.  This portion of the code is in flux and will change to more cleanly handle data precision.  The `BDF.write_bdf method(...)` is more stable.
