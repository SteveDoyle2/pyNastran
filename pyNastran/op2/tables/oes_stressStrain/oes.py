...
class OES(RealElementsStressStrain, ComplexElementsStressStrain, RealElementsStressStrain2):
    ...
    def readTable_OES_3(self, iTable):
        ...
        _buffer_words = self.get_buffer_words()
        ...
        self._parse_approach_code(data)  # 3
        ## element type
        ...
        ## stress/strain codes
        self._add_data_parameter(data, 's_code', 'i', 11, False)
        ...
        if self.analysis_code == 1:   # statics / displacement / heat flux
            ...
            self._add_data_parameter(data, 'mode_cycle', 'f', 7, False)
        ...
        #    self.lsdvmn = self.get_values(data,'i',5) ## load set number
            ...
            self._apply_data_code_value('dataNames', ['dt'])
            ...
            ## mode number
            self._add_data_parameter(data, 'lsdvmn', 'i', 5)
            self._add_data_parameter(data, 'eigr', 'f', 6, False)  # real eigenvalue
    ...
    def readOES_Data(self):
        ...
        if self.element_type in [1, 3, 10]:  # crod/ctube/conrod
            ...
            #    self.handle_results_buffer(self.OES_CTRIAX6_53_alt,resultName, name)
            else:
                self.not_implemented_or_skip()
            ...
            #    self.handle_results_buffer(self.OES_CQUAD4_95_alt,resultName, name)
            else:
                self.not_implemented_or_skip()
            del self.eid2
