...
from numpy import frombuffer, radians, sin, cos, vstack, repeat, array
import numpy as np
        ...
        # plates
            ...
            if self.format_code == -4 and self.num_wide == 30:
                ...
                if self.use_vector and is_vectorized:
                    ...
                    #if obj.itime == 0:
                    ints = frombuffer(data, dtype=self.idtype).reshape(nelements, 30)
                    ...
                    floats = frombuffer(data, dtype=self.fdtype).reshape(nelements, 30)[:, 2:]
    ...
    def _oes_cquad4_33(self, data, ndata, dt, is_magnitude_phase):
        ...
        if self.format_code == 1 and self.num_wide == 17:  # real
            ...
            if self.use_vector and is_vectorized:
                ...
                obj._times[obj.itime] = dt
                if obj.itime == 0:
                    ints = frombuffer(data, dtype=self.idtype).reshape(nelements, 15 * nnodes_all)
                    eids = ints[:, 0] // 10
                    ints[:, 0] = 0
    ...
    def _oes_shells_nonlinear(self, data, ndata, dt, is_magnitude_phase, stress_name):
        ...
        if self.format_code == 1 and self.num_wide == 13 and self.element_type in [88, 90]:  # real
            ...
            if self.use_vector and is_vectorized:
                ...
                if obj.itime == 0:
                    ints = frombuffer(data, dtype=self.idtype).reshape(nelements, 13)
                    eids = ints[:, 0] // 10
            ...
            if self.use_vector and is_vectorized:
                ...
                if obj.itime == 0:
                    try:
                        ints = frombuffer(data, dtype=self.idtype).reshape(nelements, 25)
                    except ValueError:
                        values = frombuffer(data, dtype=self.idtype)
    ...
    def _oes_shells_composite(self, data, ndata, dt, is_magnitude_phase, stress_name):
        ...
        if self.format_code == 1 and self.num_wide == 11:  # real
            ...
            if self.use_vector and is_vectorized:
                ...
                if obj.itime == 0:
                    ints = frombuffer(data, dtype=self.idtype).reshape(nelements, 11)
                    eids = ints[:, 0] // 10
                    nids = ints[:, 1]
