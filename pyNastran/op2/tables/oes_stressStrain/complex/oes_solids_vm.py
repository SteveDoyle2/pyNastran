...
class ComplexSolidVMArray(OES_Object):
            ...
            if self.is_sort1:
                for itime in range(ntimes):
                    for ieid, eid_nid in enumerate(self.element_node):
                        ...
                        (ox1, oy1, oz1, txy1, tyz1, txz1, ovm1) = t1
                        (ox2, oy2, oz2, txy2, tyz2, txz2, ovm2) = t2
                        d = t1 - t2
                        if not np.allclose([ox1.real, oy1.imag, oz1.real, txy1.real, tyz1.real, txz1.real],
                            ...
                            msg += '%-4s  (%s, %sj, %s, %sj)\n      (%s, %sj, %s, %sj)\n  dt12=(%s, %sj, %s, %sj)\n' % (
                                eid,
                                ox1.real, ox1.imag, oy1.real, oy1.imag,
                                ox2.real, ox2.imag, oy2.real, oy2.imag,
    ...
    def add_node_sort1(self, dt, eid, grid, inode, ex, ey, ez, etxy, etyz, etzx, evm):
        self.data[self.itime, self.itotal, :] = [ex, ey, ez, etxy, etyz, etzx, evm]
    ...
    def write_f06(self, f06_file, header=None, page_stamp='PAGE %s',
        ...
        for itime in range(ntimes):
            ...
            for deid, node, doxx, doyy, dozz, dtxy, dtyz, dtxz, dovm in zip(eids2, nodes, oxx, oyy, ozz, txy, tyz, txz, ovm):
                # TODO: cid not supported
                [oxxr, oyyr, ozzr, txyr, tyzr, txzr, ovmr,
                 oxxi, oyyi, ozzi, txyi, tyzi, txzi, ovmi] = write_imag_floats_13e(
                    [doxx, doyy, dozz, dtxy, dtyz, dtxz, dovm], is_mag_phase)
                if node == 0:  # CENTER
                    f06_file.write(
                        '0 %12i %11sGRID CS %2i GP\n'
                        '0   %22s  %-13s  %-13s  %-13s  %-13s  %-13s  %s\n'
                        '    %22s  %-13s  %-13s  %-13s  %-13s  %-13s  %-13s  %s\n' % (
                            deid, cid, nnodes,
                            'CENTER', oxxr, oyyr, ozzr, txyr, tyzr, txzr,
                            '', oxxi, oyyi, ozzi, txyi, tyzi, txzi, ovmr,
                    ))
                else:
                    f06_file.write(
                        '0  %22s   %-13s  %-13s  %-13s  %-13s  %-13s  %s\n'
...
class ComplexSolidStressVMArray(ComplexSolidVMArray, StressObject):
    def __init__(self, data_code, is_sort1, isubcase, dt):
