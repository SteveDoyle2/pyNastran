...
def write_ept(op2_file, op2_ascii, model: BDF, endian=b'<',
              nastran_format: str='nx', size: int=4) -> None:
    if not hasattr(model, 'properties'):
        return

    out = defaultdict(list)
    for pid, card in model.convection_properties.items():
        out[card.type].append(card)
    for pid, phbdy in model.phbdys.items():
        out[phbdy.type].append(pid)
    for pid, pelast in model.pelast.items():
        out[pelast.type].append(pid)
    for pid, pdampt in model.pdampt.items():
        out[pdampt.type].append(pid)
    for pid, pbusht in model.pbusht.items():
        out[pbusht.type].append(pid)
    for nsm_id, nsmadds in model.nsmadds.items():
        for card in nsmadds:
            out[card.type].append(card)

    #if not hasattr(model, 'nodes'):
        #return
    nproperties = (len(model.properties) + len(model.properties_mass) +
                   len(model.nsms)) + len(out)
    if nproperties == 0:
        return
    ...
    for pid, prop in model.properties_mass.items():
        out[prop.type].append(pid)
    for pid, nsms in model.nsms.items():
        for nsm in nsms:
            out[nsm.type].append(nsm)
    # for pid, prop in model.nsmadds.items():
    #     out[prop.type].append(pid)
    ...
    for name, pids in out.items():
        ...
        if name in skip_properties:  # pragma: no cover
            model.log.warning(f'skipping EPT-{name}')
            continue
...
def write_card(op2_file, op2_ascii,
               model: BDF, name, pids, spack, endian):
    op2_ascii.write('EPT-%s\n' % name)
    raise NotImplementedError(name)


def write_pbeam(name: str, pids: np.ndarray, itable: int,
...
def write_pmass(name: str, pids: np.ndarray, itable: int,
                op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
                nastran_format: str='nx') -> int:
    """writes the PMASS"""
    size = 4
    ...
    for pid in sorted(pids):
        prop = model.properties_mass[pid]
        data = [pid, prop.mass]
...
def write_pvisc(name: str, pids: np.ndarray, itable: int,
                op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
                nastran_format: str='nx') -> int:
    """writes the PVISC"""
    size = 4
...
def write_prod(name: str, pids: np.ndarray, itable: int,
               op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
               nastran_format: str='nx') -> int:
    """writes the PROD"""
    size = 4
...
def write_ptube(name: str, pids: np.ndarray, itable: int,
                op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
                nastran_format: str='nx') -> int:
    """writes the PSHEAR"""
    size = 4
...
def write_pshear(name: str, pids: np.ndarray, itable: int,
                 op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
                 nastran_format: str='nx') -> int:
    """writes the PSHEAR"""
    size = 4
...
def write_phbdy(name: str, pids: np.ndarray, itable: int,
                op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
                nastran_format: str='nx') -> int:
    """writes the PHBDY"""
    size = 4
...
def write_pbar(name: str, pids: np.ndarray, itable: int,
               op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
               nastran_format: str='nx') -> int:
    """writes the PBAR"""
    size = 4
...
def write_plplane(name: str, pids: np.ndarray, itable: int,
                  op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
                  nastran_format: str='nx') -> int:
    """writes the PLPLANE"""
    size = 4
...
def write_pgap(name: str, pids: np.ndarray, itable: int,
               op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
               nastran_format: str='nx') -> int:
    """writes the PGAP"""
    size = 4
...
def write_pshell(name: str, pids: np.ndarray, itable: int,
                 op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
                 nastran_format: str='nx') -> int:
    """writes the PSHELL"""
    size = 4
...
def write_plsolid(name: str, pids: np.ndarray, itable: int,
                  op2_file: BinaryIO, op2_ascii, model: BDF, endian: bytes=b'<',
                  nastran_format: str='nx') -> int:
    """writes the PLSOLID"""
    size = 4
...
def write_pbush1d(name: str, pids: np.ndarray, itable: int,
                  op2_file: BinaryIO, op2_ascii, model: BDF,
                  endian: bytes=b'<',
                  nastran_format: str='nx') -> int:
    """writes the PBUSH"""
...
EPT_MAP = {
    ...
    'PCOMPG': write_pcompg,
    'PSOLID': write_psolid,
    'PLSOLID': write_plsolid,
    'PBUSH': write_pbush,
    'PBUSH1D': write_pbush1d,
    'PFAST': write_pfast,
    'PGAP': write_pgap,
    'PLPLANE': write_plplane,
    'PCONV': write_pconv,
    'PELAS': write_pelas,
    'PDAMP': write_pdamp,
    'PELAST': write_pelast,
