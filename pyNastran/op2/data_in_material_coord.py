...
def data_in_material_coord(bdf: BDF, op2: OP2,
    ...
    for tria_type in tria_types:
        if tria_type not in bdf.card_count:
            continue
        # elems with MCID
        thistria = np.array([tria_type in e.type for e in elemsmcid])
        if debug:
            log.debug(f'found {ctria_type} with thistria={thistria}')

        if not np.any(thistria):
            continue
...
def _transform_shell_force(vecname: str,
                           vector, new_vector,
                           thetarad: np.ndarray, log: SimpleLogger):
    ...
    if veceids.shape[0] == vector.data.shape[1] // 5:
        log.debug(f'A: veceids.shape={veceids.shape} vector.data.shape={vector.data.shape}')
        steps = [5, 5, 5, 5, 5]
    else: # assuming  always that veceids.shape[0] == vector.data.shape[1]
        log.debug(f'B: veceids.shape={veceids.shape} vector.data.shape={vector.data.shape}')
        steps = [1]
