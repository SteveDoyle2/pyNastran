...
from pyNastran.converters.astros.astros import *
...
def _add_part_to_nastran(nastran_model: BDF,
    ...
    for etype, eids_nids in element_types.items():
        ...
        # TODO: well that's annoying...astros can have duplicate ids
        #if eid_offset:
        #eids = (eid_offset + 1 - eids_.min()) + eids_
        if map_eids:
            nastran_etype = astros_type_to_etype[etype]
        ...
        #Euler-Bernoulli beams (B23, B23H, B33, and B33H) are available only in Astros/Standard
...
def _create_nastran_nodes_elements(model: Astros,
...
def _build_rigid_ties(model: Astros,
                      nastran_model: BDF,
                      elements: Elements,
                      eid_offset: int) -> int:
    ties = model.ties
...
def _create_solid_properties(model: Astros, nastran_model: BDF,
                             log: SimpleLogger,
                             pid: int, mid: int,
    ...
    in order to match astros/calculix better.
    """
...
def _create_bar_properties(model: Astros, nastran_model: BDF,
                             log: SimpleLogger,
...
def _create_shell_properties(model: Astros, nastran_model: BDF,
                             log: SimpleLogger,
                             pid: int, mid: int, cid: int,
...
def map_mass_ids(model: Astros, nastran_model: BDF,
...
def map_bar_property_ids(model: Astros, nastran_model: BDF,
                           element_set_name: str,
...
def map_solid_property_ids(model: Astros, nastran_model: BDF,
                           element_set_name: str,
...
def map_shell_property_ids(model: Astros, nastran_model: BDF,
...
def get_eids_from_recursive_element_set(model: Astros,
...
def astros_to_nastran_filename(astros_inp_filename: str,
                               nastran_filename_out: str,
                               size: int=8,
                               xform: bool=False,
                               encoding: Optional[str]=None,
                               log: Optional[SimpleLogger]=None) -> BDF:
    if isinstance(astros_inp_filename, Astros):
        model = astros_inp_filename
    else:
        model = read_astros(astros_inp_filename, encoding=encoding, log=log, debug=True)
    log = model.log
...
def _write_frequency(model: Astros,
                     step: Step,
                     nastran_model: BDF,
...
def _write_boundary_as_nastran(model: Astros,
                               step: Step,
                               nastran_model: BDF,
...
def _create_nastran_loads(model: Astros,
...
def _write_concentrated_loads(model: Astros,
                              step: Step,
...
def _write_distributed_loads(model: Astros,
                             step: Step,
                             nastran_model: BDF, subcase: Subcase,
                             load_id: int,
                             comment_list: list[str]) -> int:
...
def _get_nodes(model: Astros, nid: int | str) -> list[int]:
...
def _get_elements(model: Astros, eid: int | str) -> list[int]:
...
def cmd_astros_to_nastran(argv=None, log: Optional[SimpleLogger]=None,
                          quiet: bool=False) -> None:
    """Interface for astros_to_nastran"""
    ...
    msg = (
        'Usage:\n'
        f'  astros_to_nastran ASTROS_INP_IN                 {other}\n'
        f'  astros_to_nastran ASTROS_INP_IN NASTRAN_BDF_OUT {other}\n'
        '  astros_to_nastran -h | --help\n'
        '  astros_to_nastran -v | --version\n'
        '\n'
        'Required Arguments:\n'
        '  ASTROS_INP_IN       path to astros.in file\n'
        '  NASTRAN_BDF_OUT     path to nastran.bdf file (default=astros.bdf)\n'
        '\n'
        ...
        'Examples:\n'
        '  # creates model.bdf\n'
        '  astros_to_nastran model.inp\n\n'
        '  # creates fem.bdf\n'
        '  astros_to_nastran model.inp fem.bdf\n\n'
        '  # creates model.bdf with large field format\n'
        '  astros_to_nastran model.inp --large\n\n'
        '  # creates model.bdf an alternate encoding\n'
        '  astros_to_nastran model.inp --encoding utf-8-sig\n\n'
        '\n'
    )
    ...
    if not quiet:  # pragma: no cover
        print(data)
    astros_inp_filename = data['ASTROS_INP_IN']
    nastran_filename_out = os.path.splitext(astros_inp_filename)[0] + '.bdf'
