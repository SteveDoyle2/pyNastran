    ...
    for load, scale in zip(loads, scale_factors):
        ...
        if load_type in loads_to_skip:
            ...
            if nnodes == 4:
                ...
                log.debug('    case=%s nnodes=%r loadtype=%r not supported' % (
            ...
            if eids_missing:
                log.error('missing PLOAD4 element ids=%s on:\n%s' % (
            ...
            for nid, c1, d1 in zip(load.node_ids, load.components, load.enforced):
                if nid in dependents_nodes:
                    ...
                    fail_count += 1
                    if fail_count < fail_count_max:
                        log.warning(f'    nid={nid} is a dependent node and has an'
                                    f' SPCD applied\n{load}')
                c1 = int(c1)
            ...
            if load_type not in cards_ignored:
                ...
                log.warning('  get_forces_moments_array - unsupported '
                            f'load.type = {load_type}')
    if fail_count:
        fail_nids_list = list(fail_nids)
        fail_nids_list.sort()
        log.warning('fail_nids = %s' % np.array(fail_nids_list))
        ...
        if elem.type in {'CTRIA3', 'CTRIA6', 'CTRIAR'}:
            ...
            for nid in elem_node_ids:
                if nid in dependents_nodes:
                    ...
                    fail_count += 1
                    if fail_count < fail_count_max:
                        log.warning(f'    nid={nid:d} is a dependent node and has a'
                                    f' PLOAD4 applied\n{load}')
                #forces[nids.index(nid)] += F
                i = nid_map[nid]
                try:
