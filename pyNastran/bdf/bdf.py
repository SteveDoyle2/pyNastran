...
class BDF_(BDFMethods, GetCard, AddCards, WriteMeshs, UnXrefMesh):
    ...
    def export_to_hdf5_file(self, hdf5_file, exporter=None):
        ...
        """
        from pyNastran.bdf.bdf_interface.hdf5_loader import dict_int_obj_attrs
        attrs = object_attributes(self, mode='both', keys_to_skip=None)
        ...
        #print(self._slot_to_type_map['elements'])
        #print('card_count =', self.card_count)

        if 'GRID' in self.card_count:
            self.log.debug('exporting nodes to hdf5')
            node_group = hdf5_file.create_group('nodes')
            grid_group = node_group.create_group('GRID')
            nids = self._type_to_id_map['GRID']
            if len(nids) == 0:
                assert len(self.nodes) == 0, len(self.nodes)
            CARD_MAP['GRID'].export_to_hdf5_vectorized(grid_group, self, nids)

        self._hdf5_export_group(hdf5_file, 'coords')
        self._hdf5_export_elements(hdf5_file)

        groups_to_export = [
            'properties', 'masses',

            # materials
            'materials', 'creep_materials', 'thermal_materials',
            #'MATS1',
            #'MATT1', 'MATT2', 'MATT3', 'MATT4', 'MATT5', 'MATT8', 'MATT9',


            # aero
            'flutters', 'trims', 'csschds', 'gusts',

            # other
            'methods', 'tables',
        ]
        for group_name in groups_to_export:
            self._hdf5_export_group(hdf5_file, group_name)
        ...
        #for etype in etypes:
                ...
                #continue
            #print("etype =", etype)
            #CARD_MAP[etype].export_to_hdf5_vectorized(h5_file, self, eids)
        ...
        for attr in dict_int_obj_attrs:
            ...
            try:
                ...
                self.log.error('cant create %r' % attr)
                raise
            self._hdf5_export_object(group, attr, dict_obj, dict_obj.keys())
        ...
        list_keys = [
            ...
            '_stored_xref_errors', 'active_filenames', 'asets', 'bsets', 'caero_ids', 'case_control_lines',
            'coord_ids', 'csets', 'element_ids', 'executive_control_lines', 'initial_superelement_models',
            'material_ids', 'mkaeros', 'monitor_points', 'node_ids', 'omits', 'property_ids', 'qsets',
            'reject_cards', 'reject_lines', 'se_bsets', 'se_csets', 'se_qsets', 'se_suport', 'special_cards',
            'suport', 'system_command_lines', 'units', 'xyz_limits'
        ]
        other_keys = [
            ...
            '_stop_on_xref_error', '_unique_bulk_data_cards', '_xref', 'active_filename',
            'aero', 'aeros', 'axic', 'axif', 'baror', 'bdf_filename', 'beamor', 'cards_to_read',
            'case_control_deck', 'debug', 'doptprm', 'dtable', 'dumplines', 'echo', 'force_echo_off',
            'grdset', 'include_dir', 'is_bdf_vectorized', 'is_long_ids', 'is_msc', 'is_nx',
            'is_superelements', 'is_zona', 'log', 'nastran_format', 'ncaeros', 'ncoords', 'nelements',
            'nid_cp_cd', 'nmaterials', 'nnodes', 'nproperties', 'point_ids', 'punch', 'radset',
            'read_includes', 'save_file_structure', 'seqgp', 'sol', 'sol_iline',
            'sol_method', 'wtmass', 'xyz_cid0', 'zona',
        ]
    ...
    def load_hdf5_file(self, h5_file):
        ...
        from pyNastran.bdf.bdf_interface.hdf5_loader import (
            hdf5_load_elements, hdf5_load_properties, hdf5_load_coords,
            hdf5_load_tables, hdf5_load_methods, hdf5_load_masses, hdf5_load_materials,
            hdf5_load_generic,
        ...
        for key in keys:
            ...
            if key == 'nodes':
                ...
                for nid, xyzi, cpi, cdi, psi, seidi in zip(nids, xyz, cp, cd, ps, seid):
                    self.add_grid(nid, xyzi, cp=cpi, cd=cdi, ps=psi, seid=seidi, comment='')
                ...
                self.log.warning('skipping hdf5 load for %s' % key)

    def _h5_export_class(self, sub_group, key, value, skip_attrs, debug=True):
        ...
        if isinstance(value, list):
            raise NotImplementedError('list: %s' % attr)
            #for valuei in value:
    ...
    def _hdf5_export_elements(self, hdf5_file):
        """
        ...
        """
        etypes_actual = []
        etypes = self._slot_to_type_map['elements']
        for card_name in self.card_count:
            #CTRIA3, CQUAD4
            #CONROD
            #CBUSH
            #CBEAM
        ...
        if etypes_actual:
            ...
            def save_solids(etype, slot_name):
                ...
                CARD_MAP[slot_name].export_to_hdf5_vectorized(element_group, self, eids)
            ...
            for card_type in etypes_actual:
                ...
                eids = self._type_to_id_map[card_type]
                class_obj = CARD_MAP[card_type]

                if hasattr(class_obj, 'export_to_hdf5_vectorized'):
                    class_obj.export_to_hdf5_vectorized(element_group, self, eids)
                else:
                    self._hdf5_export_object(element_group, card_type, self.elements, eids)

    def _hdf5_export_group(self, hdf5_file, group_name, debug=False):
        """
        exports the properties to an hdf5_file
        """
        data_dict = getattr(self, group_name) # self.properties
        if debug:
            print(group_name, data_dict)

        types_actual = []
        types = self._slot_to_type_map[group_name]
        if debug:
            print('card_count =', self.card_count)
            print('types =', types)
        for card_name in self.card_count:
        ...
        if types_actual:
            self.log.debug('exporting %s to hdf5' % group_name)
            if debug:
            ...
            for card_type in types_actual:
                ...
                class_obj = CARD_MAP[card_type]
                if hasattr(class_obj, 'export_to_hdf5_vectorized'):
                    class_obj.export_to_hdf5_vectorized(sub_group, self, ids)
                else:
                    self._hdf5_export_object(sub_group, card_type, data_dict, ids)

    def _hdf5_export_object(self, group, name, obj_dict, keys):
        i = 0
        ...
        for key in keys:
            value = obj_dict[key]
            try:
                self._h5_export_class(sub_group, key, value, skip_attrs, debug=False)
            except:
                # for debugging
                sub_group2 = group.create_group('values2')
                self._h5_export_class(sub_group2, key, value, skip_attrs, debug=True)
            i += 1
