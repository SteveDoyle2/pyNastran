...
def deform_spline(structure_model: BDF,
                  aero_model: BDF,
    ...
    for eid, spline in sorted(structure_model.splines.items()):
        ...
        if spline_type == 'SPLINE1':
            if caero_type == 'CAERO1':
                ...
                spline_nids = np.arange(len(tz_disp_local), dtype='int32')
                tz_spline_local_deflected = _map_spline1_deflections(
                    spline_nids, xyz_local, tz_disp_local, xyz_aero_local)
...
def _map_spline1_deflections(nids: np.array,
    ...
    if delta_max == 0.0:
        ...
        C = get_c_matrix(nids, xyz_structure)
        #print('nids =', nids)
        ...
        w_aero = get_w_aero(nids, C, w_structure2, xyz_structure, xyz_aero)
    return w_aero


def get_c_matrix(nids: np.ndarray,
                 xyz_structure: np.ndarray) -> np.ndarray:
    """
    Parameters
    ----------
    nids
    ...
    for inode, xyz_cidi in zip(nids, xyz_structure):
        xi, yi, zi = xyz_cidi
        ...
        j = 3
        for jnode, xyz_cidj in zip(nids, xyz_structure):
            xj, yj, zj = xyz_cidj
            if i == j:
                c_array2[i, j] = 0.
            else:
            ...
            j += 1
        i += 1
    assert np.array_equal(c_array1, c_array2)
...
def get_w_aero(nids: np.ndarray,
               C: np.ndarray,
               w_structure: np.ndarray,
               xyz_structure: np.ndarray,
               xyz_aero: np.narray) -> np.ndarray:
    #print(f'C:\n{C}')
    ...
    w_aero = get_xk(cws, nids, xyz_structure, xyz_aero)
...
def get_xk(cws: np.ndarray,
    ...
    w_aero = np.zeros(naero, dtype='float64')
    i = 0
    for iaero, aero_xyzi in enumerate(xyz_aero):
        ...
        j = 3
        for jnode, str_xyzi in zip(nids, xyz_structure):
            #(xs, ys, zs) = str_xyzi.get_position()
            (xs, ys, zs) = str_xyzi
