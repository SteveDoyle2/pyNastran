...
class ACOORD(CoordBase):  # not done
    ...
    def __init__(self, cid: int, origin: np.ndarray, delta: float, theta: float, comment: str = ""):
        """
    ...
    def rotation_x(self, ct, st):
        matrix = np.array([[1.0, 0.0, 0.0], [ct, 0.0, -st], [-st, 0.0, ct]])
        return matrix

    def rotation_y(self, ct, st):
        matrix = np.array([[ct, 0.0, st], [0.0, 1.0, 0.0], [-st, 0.0, ct]])
        return matrix

    def rotation_z(self, ct, st):
        matrix = np.array([[ct, st, 0.0], [-st, ct, 0.0], [0.0, 0.0, 1.0]])
        return matrix
...
class AEROZ(Aero):
    ...
    def add_card(cls, card: BDFCard, comment: str = ""):
        ...
        cref = double_or_blank(card, 6, "cRef", default=1.0)
        bref = double_or_blank(card, 7, "bRef", default=1.0)
        sref = double_or_blank(card, 8, "Sref", default=1.0)

        xref = double_or_blank(card, 9, "xRef", default=0.0)
        yref = double_or_blank(card, 10, "yRef", default=0.0)
        zref = double_or_blank(card, 11, "zref", default=0.0)
    ...
    def convert_to_zona(self, unused_model: BDF):
        ...
        aeros = AEROS(
            cref, bref, sref, acsid=acsid, rcsid=0, sym_xz=sym_xz, sym_xy=0, comment=str(self)
        )

        velocity = 1.0
        aero = AERO(velocity, cref, rho_ref, acsid=acsid, sym_xz=sym_xz, sym_xy=0, comment="")
        return aeros, aero
...
class ATTACH(BaseCard):
    ...
    def __init__(self, attach_id: int, model: str, setk: int, refgrid: int, comment: str = ""):
...
class MLDPRNT(BaseCard):
    ...
    def add_card(cls, card: BDFCard, comment: str = ""):
        ...
        filename = string_multifield_dollar_int_or_blank(card, (2, 3), "filename", default="")
...
class MLDSTAT(BaseCard):
    ...
    def raw_fields(self):
        ...
        list_fields = [
            ...
            self.mldtrim_id,
            self.transform,
            self.state_space_arr,
            self.state_space_brr,
            self.dx_tox,
            filename_a,
            filename_b,
        ]
...
class MINSTAT(BaseCard):
    ...
    def add_card(cls, card: BDFCard, comment: str = ""):
        ...
        filename = string_multifield_dollar_int_or_blank(card, (11, 12), "filename", default="")
...
class MLDTIME(BaseCard):
    ...
    def add_card(cls, card: BDFCard, comment: str = ""):
        ...
        return MLDTIME(mldtime_id, tstart, tend, dt, out_dt, print_flag, method, comment=comment)
...
class SPLINE0(BaseCard):
    ...
    def __init__(self, eid: int, model_name: str, cp: int, setk: int, comment: str = ""):
        BaseCard.__init__(self)
        if comment:
            self.comment = comment
        self.eid = eid
...
class TRIMFLT(BaseCard):
    ...
    def __init__(self, trimflt_id: int, title: str, alpha: float, fields: list, comment: str = ""):
        BaseCard.__init__(self)
        if comment:
            self.comment = comment
...
class CMARGIN(BaseCard):
    ...
    def add_card(cls, card, comment: str = ""):
        ...
        return CMARGIN(
            cmargin_id, gm_high, gm_low, pm_high, pm_low, df, print_flag, nroot, comment=comment
        )
...
class APCONST(BaseCard):
    ...
    def add_card(cls, card, comment: str = ""):
        ...
        for j in range(max(ncp, 1)):
            ...
            fc_values.append(fc)
            i += 1
        return APCONST(sid, da0, da1, da2, nrp, ncp, fr_values, fc_values, comment=comment)

    def raw_fields(self):
        fields = (
            ["APCONST", self.sid, self.da0, self.da1, self.da2, self.nrp, self.ncp]
            + self.fr_values
            + self.fc_values
        )
        return fields
