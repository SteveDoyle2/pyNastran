...
class OUG(TableReader):
    ...
    def _read_oug1_3(self, data, ndata):
        ...
        op2_reader.acoustic_flag = self.add_data_parameter(data, 'acoustic_flag', b'i', 13, False)
        ...
        if analysis_code == 1:   # statics / displacement / heat flux
            ...
            op2_reader.mode_cycle = self.add_data_parameter(data, 'mode_cycle', b'i', 7, False)
            op2_reader.update_mode_cycle('mode_cycle')
    ...
    def _read_oug2_3(self, data, ndata):
        ...
        self.node_id = self.add_data_parameter(data, 'node_id', b'i', 5, fix_device_code=True)
            ...
            #self.set_null_nonlinear_factor()
    ...
    def _read_oug_4(self, data, ndata):
        ...
        if table_code == 1:   # Displacements
            if table_name in [b'OUGV1', b'OUGV2',
                                   b'ROUGV1', b'ROUGV2',
                                   b'OUG1',
                                   b'BOUGV1',
                                   b'TOUGV1',
            ...
            elif table_name == b'OUGV1PAT':
                n = self._read_oug_displacement(data, ndata, is_cid=True)
            elif table_name == b'OAG1':
                n = self._read_oug_acceleration(data, ndata)
            elif table_name == b'OCRUG':
                n = self._read_oug_displacement(data, ndata, is_cid=False)
            else:
                raise NotImplementedError(op2_reader.code_information())
        elif table_code == 7:
    ...
    def _read_oug_displacement(self, data, ndata, is_cid):
        op2_reader = self.op2_reader
        op2_reader._setup_op2_subcase('displacement')

        table_name = self.op2_reader.table_name
        thermal = op2_reader.thermal
        if table_name in [b'ROUGV1', b'ROUGV2']:
            assert thermal in [0], op2_reader.code_information()
            result_name = 'displacements_ROUGV1'

        elif table_name in [b'OUG1', b'OUGV1', b'OUGV2', b'OUGV1PAT', b'BOUGV1']:
            assert thermal in [0, 1], op2_reader.code_information()
            if thermal == 0:
                result_name = 'displacements'
            elif thermal == 1:
                result_name = 'temperatures'
            else:
                msg = 'displacements; table_name=%s' % table_name
                raise NotImplementedError(msg)

        elif table_name in [b'OUPV1']:
            #result_name = 'temperatures'
            assert thermal in [2, 4, 8], op2_reader.code_information()
            if thermal == 2:
        ...
        elif table_name in [b'TOUGV1', b'TOUGV2']:
            result_name = 'temperatures'
            assert thermal == 1, op2_reader.code_information()
        elif table_name in [b'OCRUG']:
            result_name = 'displacements'
            assert thermal == 0, op2_reader.code_information()
        else:
            msg = 'displacements; table_name=%s' % table_name
            raise NotImplementedError(msg)

        storage_obj = self.get_result(result_name)
        random_code = op2_reader.random_code
        if thermal == 0:
            #result_name = 'displacements'
            ...
            assert table_name in [b'BOUGV1', b'ROUGV1', b'ROUGV2', b'OUGV1', b'OUGV2', b'OUG1', b'OCRUG'], table_name
            n = op2_reader._read_table_vectorized(data, ndata, result_name, storage_obj,
                                                  RealDisplacementArray, ComplexDisplacementArray,
                                                'node', random_code=random_code,
                                                is_cid=is_cid)
        elif thermal == 1:
            #result_name = 'temperatures'
            ...
            if self._results.is_not_saved(result_name):
                return ndata
            self._results._found_result(result_name)
            assert table_name in [b'OUGV1', b'OUGV2', b'TOUGV1', b'TOUGV2'], self.table_name
            n = op2_reader._read_scalar_table_vectorized(data, ndata, result_name, storage_obj,
            ...
            if self._results.is_not_saved(result_name):
                return ndata
            self._results._found_result(result_name)
            assert table_name in [b'OUPV1'], table_name
            n = op2_reader._read_table_vectorized(data, ndata, result_name, storage_obj,
                                                  RealDisplacementArray, ComplexDisplacementArray,
                                                  'node', random_code=random_code)
        elif thermal == 4:
            ...
            if self._results.is_not_saved(result_name):
                return ndata
            self._results._found_result(result_name)
            assert self.table_name in [b'OUPV1'], table_name
            n = op2_reader._read_table_vectorized(data, ndata, result_name, storage_obj,
                                                  RealDisplacementArray, ComplexDisplacementArray,
                                                  'node', random_code=random_code)
        elif thermal == 8:  # 4 ?
            ...
            n = self._not_implemented_or_skip(data, ndata, 'bad thermal=%r table' % thermal)
        #else:
            #raise NotImplementedError(thermal)
        return n
