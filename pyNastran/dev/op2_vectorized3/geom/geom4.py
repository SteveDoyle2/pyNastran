...
from pyNastran.bdf.cards.bdf_sets import (
    USET, USET1,
    #ASET, ASET1, BSET, BSET1, CSET, CSET1, QSET, QSET1, SEQSET1,
    #OMIT1, # SEQSET
)
...
class GEOM4(GeomCommon):
    ...
    def __init__(self, op2: OP2Geom):
        ...
        self.geom4_map = {
            ...
            # ???
            (4901, 49, 310017) : ['MPC-3', self._read_mpc],
    ...
    def _read_mpc(self, data: bytes, n: int) -> int:
        ...
        while i < nfields:
            ...
            op2.add_mpc(sid, nodes, components, coefficients)
    ...
    def _read_rbar_nx_28(self, card_obj, data: bytes, n: int) -> int:
        ...
        n, ints = get_ints(data, n, nelements, 7,
                           size=op2.size, endian=op2._endian)
        element = op2.rbar
        element_id = ints[:, 0]
        nodes = ints[:, [1, 2]]
        independent_dof = ints[:, [3, 4]]
        dependent_dof = ints[:, [5, 6]]
        assert element_id.min() > 0, element_id
        element._save(element_id, nodes, independent_dof, dependent_dof,
                      alpha=None, tref=None)
    ...
    def _read_rbar_msc_36(self, card_obj, data: bytes, n: int) -> int:
        ...
        n, ints, floats = get_ints_floats(data, n, nelements, 9,
                                              size=op2.size, endian=op2._endian)
        element = op2.rbar
        element_id = ints[:, 0]
        nodes = ints[:, [1, 2]]
    ...
    def _read_rrod_nx(self, data, n):
        ...
        element._save(element_id, nodes, cm, alpha=None)
    ...
    def _read_spc_msc(self, data, n: int):
        ...
        assert spc_id.min() > 0, spc_id
        constraint._save(spc_id, node_id, components, enforced)
    ...
    def _add_spc1_card(self, out):
        ...
        if thru_check and len(out) > 5:
            ...
            op2.add_spc1(sid, components, nids)
    ...
    def _read_suport(self, data: bytes, n: int) -> int:
        ...
        for unused_i in range(nentries):
            ...
            if op2.is_debug_file:
                ...
                #op2.log.info(out)
            nid, comp = out
            suport = op2.add_suport(nid, comp)
    ...
    def _read_suport1(self, data: bytes, n: int) -> int:
        ...
        while i < len(out):
            if out[i] == -1:
                ...
                #print(suport)
                sid = suport[0]
                assert len(suport) % 2 == 1, suport
                nids = []
                comps = []
                for j in range(1, len(suport), 2):
                    nid = suport[j]
                    comp = suport[j+1]
                    nids.append(nid)
                    comps.append(str(comp))
                #nid, comp = suport
                #print(sid, nids, comps)
                op2.add_suport1(sid, nids, comps)
        ...
        #print('***suport', suport)
        assert len(suport) == 3, suport
        sid, nid, comp = suport
        op2.add_suport1(sid, [nid], [str(comp)])
        #suporti = SUPORT1.add_op2_data(suport)
        #op2._add_methods.add_suport_object(suporti) # extracts [sid, nid, c]
        nsuports += 1
    ...
    def _read_uset1(self, data: bytes, n: int) -> int:
        ...
        while i < nidata:
            ...
            uset1.add(sname_str, nids, comp_str)
...
def read_rbe2s_from_idata_fdata(op2: OP2Geom, idata, fdata) -> list[RBE2]:
    ...
    for ii, jj in zip(i, j):
        ...
        op2.add_rbe2(eid, gn, cm, gm, alpha=alpha,
                     tref=0.0, comment='', validate=False)
        #rbe2s.append(elem)
...
def read_rbe3s_from_idata_fdata(op2: OP2Geom, idata, fdata) -> list[RBE3]:
    ...
    for ii, jj in zip(i, j):
        ...
        #print('rbe3 =', in_data)
        if is_alpha_tref:
            op2.add_rbe3(eid, refg, refc, weights, comps, gijs,
                         Gmi=gmi, Cmi=cmi, alpha=alpha, tref=tref, comment='')
        elif is_alpha:
            op2.add_rbe3(eid, refg, refc, weights, comps, gijs,
                         Gmi=gmi, Cmi=cmi, alpha=alpha, comment='')
        else:
            op2.add_rbe3(eid, refg, refc, weights, comps, gijs,
                         Gmi=gmi, Cmi=cmi, comment='')
