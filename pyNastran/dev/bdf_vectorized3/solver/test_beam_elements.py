...
sys.path.insert(0, str(Path(r"C:\work\code\claude_code")))
...
def test_kgg_vs_reference_timoshenko():
    """KGG matches timoshenko_beam2.beam_kgg at machine precision."""
    from timoshenko_beam2 import beam_kgg
...
def test_kgg_cantilever_deflection():
...
def test_kdgg_vs_reference():
    """KDGG matches timoshenko_beam2.beam_kdgg (NX Nastran EB formula)."""
    from timoshenko_beam2 import beam_kdgg
...
def test_consistent_mass_total():
...
def test_consistent_mass_symmetric_positive():
...
# =============================================================================
# PG (load vector) tests
# =============================================================================


def test_pg_distributed_vs_reference():
    """beam_pg_distributed matches timoshenko_beam2.beam_pg."""
    from timoshenko_beam2 import beam_pg
    ...
    qz = 1000.0
    fe_ref = beam_pg(0, 0, qz, L, E, G, A, Iy, Iz, ks_y=1.0, ks_z=1.0)
...
def test_pg_point_vs_reference():
    """beam_pg_point matches timoshenko_beam2.beam_pg_point."""
    from timoshenko_beam2 import beam_pg_point as ref_pg_point
...
def test_recover_cantilever_forces():
...
def test_stress_recovery_axial():
...
def test_buckling_euler_column():
...
# =============================================================================
# Integration tests: SOL 101 CBAR/CBEAM with analytical verification
# =============================================================================


def test_sol101_cbar_axial():
    ...
    bdf_path = str(Path(__file__).parent / "cbar.solver.bdf")
    model = BDF3(log=None)
...
def test_sol101_cbeam_runs():
    """CBEAM SOL 101 runs without error on the reference BDF."""
    from pyNastran.dev.bdf_vectorized3.bdf import BDF as BDF3
    from pyNastran.dev.bdf_vectorized3.solver.solver import Solver

    bdf_path = str(Path(__file__).parent / "cbeam.solver.bdf")
    model = BDF3(log=None)
    model.read_bdf(bdf_path)
    solver = Solver(model)
    solver.run()
...
def test_sol101_cbar_reference_bdf():
...
def test_kgg_shear_relief():
    """KGG with S1/S2 shear relief matches timoshenko_beam.py reference."""
    from timoshenko_beam import beam_stiffness_3d as ref_stiffness
...
def test_kgg_shear_relief_zero_matches_plain():
...
def test_sol101_cbar_combined_loading():
