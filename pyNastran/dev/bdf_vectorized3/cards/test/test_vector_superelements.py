...
class TestSuperelements(unittest.TestCase):
    def _test_superelements_1(self):
    ...
    def _test_superelement_setree(self):
        """tests the SETREE"""
        model = BDF(debug=False)
...
def create_superelement(debug=False) -> BDF:
    """creates a simple bar model"""
    super_element = BDF(debug=debug)
    super_element.add_grid(10, [0., 0., 0.])
    super_element.add_grid(20, [0., 0., 1.])
    super_element.add_grid(30, [1., 0., 0.])

    super_element.add_grid(11, [1., 1., 0.])
    super_element.add_grid(12, [2., 2., 0.])

    x = [0., 0., 1.]
    g0 = None
    super_element.add_cbar(100, 1000, [11, 12], x, g0)
    super_element.add_pbarl(1000, 2000, 'ROD', [1.,])
