...
from pyNastran.bdf.bdf_interface.utils import (
    _parse_pynastran_header, to_fields,
...
from pyNastran.bdf.bdf_interface.internal_bdf import (
    get_file_tag, expand_replication, check_replicated_cards)
...
from pyNastran.bdf.cards.contact import (
    BCRPARA, BCPARA, BCTPARA, BCTPARM) # BLSEG, BCBODY
...
class BDF(AddCards, WriteMesh): # BDFAttributes
    ...
    def save_obj(self, obj_filename: PathLike='model.obj', unxref: bool=True) -> None:
    ...
    def load_obj(self, obj_filename: PathLike='model.obj') -> None:
    ...
    def pop_parse_errors(self) -> None:
        ...
        if self._stop_on_parsing_error:
            ...
            is_error = False
            msg = ''
            # if duplicates := self._duplicate['elements']:
            #     duplicate_eids = [elem.eid for elem in duplicates]
            ...
            #         msg += 'new_elements=\n'
            #         for elem, eidi in zip(duplicates, duplicate_eids):
            #             if eidi == eid:
            #                 msg += str(elem)
            #         msg += '\n'
            #         is_error = True
            ...
            #
            # if duplicates := self._duplicate['properties']:
            #     duplicate_pids = [prop.pid for prop in duplicates]
            ...
            #         msg += 'new_properties=\n'
            #         for prop, pidi in zip(duplicates, duplicate_pids):
            #             if pidi == pid:
            #                 msg += str(prop)
            #         msg += '\n'
            #         is_error = True
            #
            # if duplicates := self._duplicate['masses']:
            #     duplicate_eids = [elem.eid for elem in duplicates]
            #     uduplicate_eids = np.unique(duplicate_eids)
            ...
            #         is_error = True
            #
            # if duplicates := self._duplicate['materials']:
            #     duplicate_mids = [mat.mid for mat in duplicates]
            ...
            #         msg += 'new_materials=\n'
            #         for mat, midi in zip(duplicates, duplicate_mids):
            #             if midi == mid:
            #                 msg += str(mat)
            #         msg += '\n'
            #         is_error = True
            #
            # if duplicates := self._duplicate['thermal_materials']:
            #     duplicate_mids = [mat.mid for mat in duplicates]
            #     uduplicate_mids = np.unique(duplicate_mids)
            ...
            #         is_error = True
            #
            # if duplicates := self._duplicate['coords']:
            #     duplicate_cids = [coord.cid for coord in duplicates]
            #     uduplicate_cids = np.unique(duplicate_cids)
            #     msg += 'self.coords IDs are not unique=%s\n' % uduplicate_cids
            #     for cid in uduplicate_cids:
            #         msg += 'old_coord=\n%s\n' % str(self.coords[cid])
            #         msg += 'new_coords=\n'
            #         for coord, cidi in zip(duplicates, duplicate_cids):
            #             if cidi == cid:
            #                 msg += str(coord)
            #         msg += '\n'
            #         is_error = True
            ...
            if card_name and card_name[0] not in ['+', '*']:
                if old_card_name:
                    ...
                    if not self.allow_tabs and '\t' in (joined_lines_n := '\n'.join(card_lines)):
                        tag = get_file_tag(self, ifile_iline)
                        raise RuntimeError(f'There are tabs in:\n{joined_lines_n}{tag}')
    ...
    def _make_card_parser(self) -> None:
        ...
        self._card_parser_prepare = {
            ...
            # supports 2 cards in 1
            'CORD1R': self._prepare_cord1r,
            'CORD1C': self._prepare_cord1c,
            'CORD1S': self._prepare_cord1s,
            'CORD2R': partial(self._prepare_card_by_method, self.coord.add_cord2r_bdf),
            'CORD2C': partial(self._prepare_card_by_method, self.coord.add_cord2c_bdf),
            'CORD2S': partial(self._prepare_card_by_method, self.coord.add_cord2s_bdf),
            ...
            'PSOLCZ': partial(self._prepare_card, self.psolcz),
            'PCOMPS': partial(self._prepare_card, self.pcomps),
            'PCOMPLS': partial(self._prepare_card, self.pcompls),

            # cohesive zone solids
            #nx
            'CPENTCZ' : partial(self._prepare_card, self.cpentcz),
