...
class CTRIA5(FEM):
    ...
    def verifyN(self,N,V):
        v = [1,0,0]; assert self.subABC(N,1,0,0      ) == V[0],'f(%s)=%s = %s' %(v,N,self.subABC(N,1,0,0    ))
        v = [0,1,0]; assert self.subABC(N,0,1,0      ) == V[1],'f(%s)=%s = %s' %(v,N,self.subABC(N,0,1,0    ))
        v = [0,0,1]; assert self.subABC(N,0,0,1      ) == V[2],'f(%s)%s = %s'  %(v,N,self.subABC(N,0,0,1    ))
        v = [.5,.5,0]; assert self.subABC(N,0.5,0.5,0) == V[3],'f(%s)%s = %s'  %(v,N,self.subABC(N,0.5,0.5,0))
        v = [0,.5,.5]; assert self.subABC(N,0,0.5,0.5) == V[4],'f(%s)%s = %s'  %(v,N,self.subABC(N,0,0.5,0.5))
