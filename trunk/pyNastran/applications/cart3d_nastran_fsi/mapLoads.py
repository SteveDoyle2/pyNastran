...
class LoadMapping(object):
    ...
    def setFlightCondition(self,pInf=499.3,qInf=237.885):
        self.pInf = pInf
    ...
    def mapLoads(self):
        ...
        sys.stdout.flush()
        for aEID,distribution in self.mappingMatrix.items():
            #print "aEID = ",aEID
            ...
            sumForces  += Fn
            for sNID,percentLoad in sorted(distribution.items()):
                sumLoad += percentLoad
        ...
        #self.bdf.write('$***********\n')
        log().info("wrote loads to |%s|" %(self.bdffile))
    ...
    def writeLoads(self):
        ...
        for loadCase,loads in sorted(self.loadCases.items()):
            log().info("  loadCase = %s" %(loadCase))
            for (sNID,Fxyz) in sorted(loads.items()):
    ...
    def parseMapFile(self,mapFilename='mappingMatrix.new.out'):
        ...
        log().info('loading mapFile=|%s|' %(mapFilename))
    ...
    def runMapTest(self,mappingMatrix,mapTestFilename='mapTest.out'):
        ...
        """
        mapTest = {}
        for (aEID,distribution) in sorted(mappingMatrix.items()):
        ...
        for sEID,weight in sorted(mapTest.items()):
            mapOut.write("%s %s\n" %(sEID,weight))
        mapOut.close()
    ...
    def buildMappingMatrix(self,debug=False):
        ...
        for (i,aEID) in enumerate(aElementIDs):
            ...
            (aArea,aCentroid,aNormal) = aModel.getElementProperties(aEID)
            ...
            tLeft = tEst-dt
            log().debug("tEst=%g minutes; tLeft=%g minutes" %(tEst,tLeft))
        ###
        mapFile.close()
    ...
    def poorMansMapping(self,aCentroid,aEID,pSource,normal):
        ...
        for sEID in sElements:
            sNodes = self.structuralModel.getElementNodes(sEID)
            setNodes.union(set(sNodes))

        nIDs = list(setNodes)
        sNodes = sModel.getNodeIDLocations(nIDs)
        weighs = self.getWeights(closePoint,sNodes)
    ...
    def pierceElements(self,aCentroid,aEID,pSource,normal):
        ...
        *P = A+(B-A)*t
        ...
        for sEID,sDist in zip(sElements,sDists):
            ...
            (sArea,sNormal,sCentroid) = self.structuralModel.getElementProperties(sEID)
            sNodes = self.structuralModel.getElementNodes(sEID)
            nNodes = len(sNodes)
    ...
    def getWeights(closePoint,nodes):
        # TODO: new weights?
    ...
    def distributeUnitLoad(self,aEID,piercedElements,nPiercings):
        ...
        if nPiercings==0:
            ...
            for sEID in piercedElements:
                nIDs += sModel.getElementNodeIDs(sEID)
            #print "nIDs1 = ",nIDs
            nIDs = list(set(nIDs))
            ...
            for closeElement in reversed(closeElements):
                ...
                setElementNodes = set(sModel.getElementNodeIDs(sEID))
...
def runMapLoads(cart3dGeom='Components.i.triq',bdfModel='fem.bdf',bdfModelOut='fem.loads.out'):
    assert os.path.exists(bdfModel),'|%s| doesnt exist' %(bdfModel)

    t0 = time()
    mesh = Cart3DReader(cart3dGeom)
    (nodes,elements,regions,Cp) = mesh.read()
    (nodes,elements,regions,Cp) = mesh.makeHalfModel(nodes,elements,regions,Cp)
    ...
    fem = FEM_Mesh(bdfModel)
    fem.read(debug=False)
    sys.stdout.flush()
    ...
    t1 = time()
    mapper.setFlightCondition(pInf,qInf)
...
if __name__=='__main__':
    ...
    runMapLoads(cart3dGeom,bdfModel,bdfModelOut)
